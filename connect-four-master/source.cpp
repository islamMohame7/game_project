...
unsigned int player2 = 2; // AI number
...
void playGame() {
	...
	printBoard(board); // print initial board
      cout<<"hello "<<endl;
      cout<<"choose 1 to play with computer" <<endl;
      cout<<"choose 2 to play with participant" <<endl;
      cin>>ui;
      if (ui==1){
	...
	if (turns == NUM_ROW * NUM_COL) { // if draw condition
    	...
    	while (!gameOver) {
                if (turns%2==0){
            cout<<"user 1 move"<<endl;
            makeMove(board, userMove(), PLAYER);
                }
                else if (turns%2!=0) {
            cout<<"user 2 move"<<endl;
            makeMove(board, userMove(), player2);
                }
	...
	if (turns == NUM_ROW * NUM_COL) { // if draw condition
		...
		cout << ((currentPlayer == PLAYER) ? "user two win!" : "user one Wins!") << endl;
	}
    	}
